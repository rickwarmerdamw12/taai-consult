...
Deno.serve(async (req) => {
  try {
    ...
    const contactSubmissionId =
      payload.entity_id || payload.event?.entity_id || payload.contactSubmissionId;
    ...
    const contactSubmission =
      await base44.asServiceRole.entities.ContactSubmission.get(contactSubmissionId);
    ...
    const respText = await zapierResponse.text();
    console.log("Zapier status:", zapierResponse.status);
    console.log("Zapier response:", respText);
